...
export const Route = createFileRoute("/login_/coach")({
    ...
    void (async () => {
      try {
        ...
        if (result.kind === "ready") {
          ...
          navigate({ to: "/trainer/dashboard", replace: true });
          return;
        }

        if (result.kind === "missing_profile") {
          setMessage("Your account is signed in but has no profile. Contact support.");
        }
      } catch {
        if (!cancelled) {
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col justify-center px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div className="space-y-6">
            <p className="text-xs tracking-[0.3em] text-silver-muted">COACH ACCESS</p>
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl text-silver-gradient leading-[0.95]">
              COACH LOGIN
            </h1>
            <p className="max-w-xl text-sm sm:text-base text-muted-foreground">
              Access your coaching dashboard to manage clients, create programs, and track progress.
            </p>
